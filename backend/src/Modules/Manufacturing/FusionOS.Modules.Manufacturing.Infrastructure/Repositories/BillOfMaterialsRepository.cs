...
public sealed class BillOfMaterialsRepository : IBillOfMaterialsRepository
{
    ...
    // .Include(x => x.Lines) is required because the Lines collection is backed by a private field.
    public Task<BillOfMaterials?> GetByIdAsync(Guid companyId, Guid billOfMaterialsId, CancellationToken cancellationToken = default) =>
        _context.BillsOfMaterials.Include(b => b.Lines).FirstOrDefaultAsync(b => b.CompanyId == companyId && b.Id == billOfMaterialsId, cancellationToken);
