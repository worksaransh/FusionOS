...
public sealed class UpdateContactCommandHandler : IRequestHandler<UpdateContactCommand, ContactDto>
{
    ...
    {
        var contact = await _contactRepository.GetByIdAsync(request.CompanyId, request.ContactId, cancellationToken)
        ...
        contact.UpdateDetails(request.Name, request.Email, request.Phone, request.Title, request.AccountId, request.LeadId);
